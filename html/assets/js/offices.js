...
if (typeof ymaps !== 'undefined') {
	var officesAtms = {
		...
		init: function init() {
			...
			officesAtms.filterFormSubmitInit(); // officesAtms.getData();
			officesAtms.listType = $('[listType]').attr('listType');
			ymaps.ready(function() {
				function GeolocationButton(params) {
					GeolocationButton.superclass.constructor.call(this, params); // Расширяем опции по умолчанию теми, что передали в конструкторе.
					this.geoLocationOptions = ymaps.util.extend({ // Не центрировать карту.
						noCentering: false, // Не ставить метку.
						noPlacemark: false, // Не показывать точность определения местоположения.
						noAccuracy: false, // Режим получения наиболее точных данных.
						enableHighAccuracy: true, // Максимальное время ожидания ответа (в миллисекундах).
						timeout: 10000, // Максимальное время жизни полученных данных (в миллисекундах).
						maximumAge: 1000
					}, params.options);
				}
				...
				officesAtms.itemsListMap.controls // Кнопка изменения масштаба
					.add('zoomControl');
				...
				var btnTop2 = new ymaps.control.Button({
					...
					layout: ymaps.templateLayoutFactory.createClass( // Если кнопка не нажата, к ней применится css-стиль 'myButton'
						// Если кнопка нажата, к ней применятся css-стили 'myButton' и 'myButtonSelected'.
						"<div class='btn-standart -form -green js-btn-reg' style='cursor: pointer'>" + "$[data.content]" + "</div>")
				});
				btnTop2.events.add('click', function() { //officesAtms.itemsListMap.behaviors.enable('drag');
					if (navigator.geolocation) { // Запрашиваем текущие координаты устройства.
						navigator.geolocation.getCurrentPosition(ymaps.util.bind(function(position) {
								...
								/*,
																		...
																		circle = this._circle;*/ // Смена центра карты (если нужно)
								...
								/*this._placemark = */
								placemark = new ymaps.Placemark(location, {}, {
									preset: 'geolocation#icon'
								});
								map.geoObjects.add(placemark); // Показываем адрес местоположения в хинте метки.
							...
							}, this), ymaps.util.bind(function(err) {
								officesAtms.itemsListMap.hint.show(err.toString()).hide(2000);
							}, this)
							/*,
				...
				}).add('deselect', function() { //officesAtms.itemsListMap.behaviors.disable('drag');
				});
				officesAtms.itemsListMap.controls.add(btnTop2, {
					left: 5,
					top: 5
				});
		...
		filterFormSubmitInit: function filterFormSubmitInit() {
		...
		showDetailMapInit: function showDetailMapInit(itemsList) {
		...
		changeFilterInit: function changeFilterInit() {
			...
			$('[id^="filters_typeList"]').change(function() {
				...
				$('[id^="button"]').show();
				if ($(this).val() == 'Бизнес') {
					$('[id^="departmentf"]').show(); //$('[id^="filters_typeList"]  :first').attr("selected", "selected");
		...
		showItemsInMap: function showItemsInMap() {
			ymaps.ready(function() {
				...
				if (officesAtms.itemsListCollection != false) {
					officesAtms.itemsListCollection.removeAll(); // Удаляем коллекцию с карты
				...
				var itemsListCollection = officesAtms.itemsListCollection; // Определяем границы карты: координаты левого верхнего и правого нижнего ее углов.
				...
				$(document).on('click', '.b-atm-link-atms', function(e) {
					...
					$("a[name=" + name + "]").closest('.b-atm').find('.b-office-full').slideDown(function() {
						$("a[name=" + name + "]").closest('.b-atm').addClass('b-office_open');
					});
				});
			});
		},
		showItemDetailInit: function showItemDetailInit() {
		...
		showItemDetail: function showItemDetail(itemId, itemType) {}
	};
	$(function() {
		officesAtms.init();
	});
